...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', type=str, default="bilstm", help="bilstm")
    ...
    if args.model == "bilstm":
        model = BiLSTM.Model(vocab_size=len(vocab),
                             ...
                             tag_size=len(type2label))
    else:
        raise ValueError("Model should be bilstm, {} is invalid.".format(args.model))
    ...
    for i in range(args.epochs):
        ...
        train_loss, _, train_p, train_r, train_f1 = trainer.run(train_loader)
        train_list.append(train_loss)
        dev_loss, _, dev_p, dev_r, dev_f1 = trainer.run(dev_loader, train=False)
        dev_list.append(dev_loss)
        print("Train Loss: {:.4f} Precision: {:.4f} Recall: {:.4f} f1: {:.4f}".format(train_loss, train_p, train_r, train_f1))
        print("Dev   Loss: {:.4f} Precision: {:.4f} Recall: {:.4f} f1: {:.4f}".format(dev_loss, dev_p, dev_r, dev_f1))
        if dev_f1 > best_f1:
            best_f1 = dev_f1
    ...
    test_loss, _, test_p, test_r, test_f1 = trainer.run(test_loader, train=False)
    print("Test   Loss: {:.4f} Precision: {:.4f} Recall: {:.4f}  f1: {:.4f}".format(test_loss, test_p, test_r, test_f1))
